...
При расчете учитывается только одна компонента флюида, что ограничивает его
...
Здесь $\alpha_k$, $\rho_k$, $\mathbold{u}_k$, $P_k$, $E_k$ --- объемная доля,
...
В качестве уравнения состояния для каждой из фаз  используется уравнение состояния жесткого газа (stiffened-gas equation of state): 
$$
	\mathbb{U}_k = \frac{P_k+\gamma P_o}{\rho (\gamma -1 )}
$$ {#eq:stgaseos}
Корелляции для расчета межфазного обмена импульсом приведены в статье [@Houim_2016].
